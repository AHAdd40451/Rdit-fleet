-- Fixed version: Create push_tokens table to store Expo push notification tokens
-- This version uses simpler RLS policies that don't require querying the users table

-- Drop existing policies if they exist (run this first if updating)
...
-- These policies allow users to manage their own push tokens based on user_id
-- without requiring access to the users table
-- ============================================================================

-- Policy: Users can view their own push tokens
-- This uses a security definer function to check user_id without exposing users table
...
DECLARE
  ...
  user_exists BOOLEAN;
BEGIN
  -- Get current user's email/phone from auth
...
-- Uncomment this section and comment out the policies above if needed
-- ============================================================================

-- For development/testing, you can temporarily use these simpler policies:
-- They allow any authenticated user to manage push tokens for any user_id
-- (You should restrict this in production)

/*
-- Allow authenticated users to manage push tokens
